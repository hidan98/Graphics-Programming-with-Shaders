...
HoriszontalBlurShader::HoriszontalBlurShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"HorizontalBlur_vs.cso", L"HorizontalBlur_ps.cso");
}


HoriszontalBlurShader::~HoriszontalBlurShader()
{
	if (sampleState)
	{
		sampleState->Release();
...
void HoriszontalBlurShader::setShderParameters(ID3D11DeviceContext * deviceContext, const XMMATRIX & world, const XMMATRIX & view, const XMMATRIX & projection, ID3D11ShaderResourceView * texture, float width, int times)
{
...
void HoriszontalBlurShader::initShader(WCHAR * vsFilename, WCHAR * psFilename)
{
